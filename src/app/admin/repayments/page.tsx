...
import { confirmRepayment, getRepaymentQueue } from "@/lib/admin/api";
...
export default function AdminRepaymentsPage() {
  const query = useQuery({ queryKey: adminKeys.repayments, queryFn: getRepaymentQueue });
  const mutation = useAdminMutation(
    ({ id, action, reason }: { id: string; action: string; reason: string | null }) =>
      confirmRepayment(id, action, reason),
    [adminKeys.repayments, adminKeys.kpis, adminKeys.loans],
  );
  ...
  return (
    <>
      <AdminPageHeader
        eyebrow="Caisse"
        title="File des remboursements"
        description="Confirmez les remboursements effectivement reçus ou rejetez-les avec un motif traçable."
      />
      {query.isPending ? <AdminLoading /> : null}
      {query.isError ? <AdminError message={query.error.message} /> : null}
      {query.data?.length === 0 ? <AdminEmpty label="Aucune demande de remboursement" /> : null}
